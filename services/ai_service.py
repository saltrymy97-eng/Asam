...
3. عند قبض مبلغ من عميل:
   المدين = النقدية
   الدائن = العملاء

4. عند دفع مبلغ لمورد:
   المدين = الموردين
   الدائن = النقدية

5. عند سداد مصروف (كهرباء، إيجار، رواتب):
   المدين = المصروف
   الدائن = النقدية

6. الأصول تزيد بطرف مدين، والالتزامات تزيد بطرف دائن.

المطلوب هو استخراج البيانات التالية بصيغة JSON:
{
    "operation_type": "بيع، شراء، قبض، دفع، سداد مصروف، إلخ",
    "lines": [
        {
            "account": "اسم الحساب بالعربية (مثلاً: النقدية، المخزون، المبيعات، الموردين، الرواتب، إلخ)",
