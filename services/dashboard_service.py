# services/dashboard_service.py – منطق لوحة المعلومات والمؤشرات
import sqlite3
...
def get_kpi_cards():
    ...
    total_qty = conn.execute("SELECT SUM(quantity) as total FROM products").fetchone()["total"] or 0
    total_sales = conn.execute("SELECT SUM(total) as total FROM invoices WHERE type='sale'").fetchone()["total"] or 0
    total_purchases = conn.execute("SELECT SUM(total) as total FROM invoices WHERE type='purchase'").fetchone()["total"] or 0
...
def get_top_products(limit=5):
    ...
    conn.row_factory = sqlite3.Row
    rows = conn.execute("""
        SELECT p.name, SUM(ii.quantity) as total_qty, SUM(ii.quantity * ii.unit_price) as total_sales
