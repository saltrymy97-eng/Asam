...
def show():
    ...
    with tab1:
        ...
        prod_name = st.selectbox("المنتج", [p['name'] for p in products], key="fifo_batch_prod")
    ...
    with tab2:
        ...
        prod_name = st.selectbox("المنتج", [p['name'] for p in products], key="fifo_consume_prod")
        prod_id = next(p['id'] for p in products if p['name'] == prod_name)
        ...
        if st.button("تسجيل الصرف"):
            if qty_consume <= 0:
                ...
                total_cost, error = consume_fifo(prod_id, qty_consume,
    ...
    with tab3:
        ...
        prod_name = st.selectbox("المنتج", [p['name'] for p in products], key="fifo_view_prod")
        prod_id = next(p['id'] for p in products if p['name'] == prod_name)
