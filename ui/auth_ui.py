...
def apply_ultra_premium_css():
    ...
    st.markdown(f"""
<style>
    /* 🌟 استيراد خط تجوال العربي الفاخر */
    ...
    * {{
    ...
    div[data-testid="stVerticalBlock"] > div > div > div[data-testid="stVerticalBlock"] {{
        background: linear-gradient(145deg, rgba(15, 23, 42, 0.7) 0%, rgba(8, 13, 24, 0.9) 100%) !important;
    ...
    @keyframes floatAndPulse {{
        0% {{ 
            ...
            box-shadow: 0 0 40px rgba(124, 58, 237, 0.4), inset 0 10px 20px rgba(124, 58, 237, 0.3), inset 0 -10px 20px rgba(37, 99, 235, 0.3); 
        }}
        50% {{ 
            transform: translateY(-20px) rotateX(-5deg) rotateY(10deg); 
            box-shadow: 0 0 80px rgba(167, 139, 250, 0.8), 0 30px 50px rgba(0,0,0,0.8), inset 0 15px 30px rgba(124, 58, 237, 0.6), inset 0 -15px 30px rgba(37, 99, 235, 0.5); 
        }}
        100% {{ 
            transform: translateY(0) rotateX(5deg) rotateY(-5deg); 
            box-shadow: 0 0 40px rgba(124, 58, 237, 0.4), inset 0 10px 20px rgba(124, 58, 237, 0.3), inset 0 -10px 20px rgba(37, 99, 235, 0.3); 
    ...
    .hukma-orb {{
        ...
        animation: floatAndPulse 5s ease-in-out infinite; 
    ...
    .hukma-orb::after {{
        ...
        animation: laserSweep 4s cubic-bezier(0.25, 1, 0.5, 1) infinite;
        pointer-events: none;
        z-index: 1;
    }}
...
def render_premium_header(is_change_password=False):
    """توليد الهيدر التنفيذي الفاخر - بدون مسافات بادئة لضمان العرض الصحيح"""
...
<div style="text-align:center; margin-bottom: 3.5rem; margin-top: 0.5rem; perspective: 1000px;">
    ...
    <!-- 📝 الشعار اللفظي -->
    <p style="color:{S}; margin-top: 1.5rem; font-size: 1.35rem; letter-spacing: 1px; font-weight: 700;">
    ...
    <div style="width: 200px; height: 2px; background: linear-gradient(90deg, transparent, #A78BFA, transparent); margin: 1.5rem auto 0 auto; opacity: 0.8; box-shadow: 0 0 10px #A78BFA;"></div>
</div>
""", unsafe_allow_html=True)
    else:
        st.markdown(f"""
<div style="text-align:center; margin-bottom: 3.5rem; margin-top: 0.5rem; perspective: 1000px;">
    <div class="hukma-orb" style="width: 130px; height: 130px; border-radius: 50%;">
...
def login_form():
    ...
    # 🥇 توقيع المطور الفاخر بدون مسافات بادئة
    st.markdown(f"""
