...
def apply_ultra_premium_css():
    """حقن نظام التصميم السيادي والـ Cyan Glassmorphism لصفحة الدخول (نسخة آمنة ومستقرة)"""
    st.markdown(f"""
    <style>
        /* 1. خلفية كونية متحركة بنعومة */
        ...
        /* 2. حاوية الزجاج البركاني (استهداف آمن ومتوافق مع الجوال والكمبيوتر) */
        ...
        /* 4. أزرار الدخول (بتصميم يتمدد تلقائياً ليلائم الشاشة) */
        ...
        button[kind="primary"]:hover {{
            ...
            box-shadow: 0 20px 40px -10px rgba(0, 210, 255, 0.5) !important;
        ...
        /* 5. تأثير النبض الضوئي المستقر للشعار */
        @keyframes executiveGlow {{
            0%, 100% {{ filter: drop-shadow(0 0 20px rgba(0, 210, 255, 0.2)); transform: translateY(0); }}
            50% {{ filter: drop-shadow(0 0 40px rgba(0, 82, 212, 0.4)); transform: translateY(-3px); }}
        }}
        .executive-logo-box {{
...
def render_premium_header(is_change_password=False):
    ...
    if not is_change_password:
        ...
        <div style="text-align:center; margin-bottom: 2rem; margin-top: 0.5rem;">
            ...
            ">
                <span style="font-size: 2.8rem; font-weight: 950; 
            ...
            <h1 style="color:{T}; font-size: 2.5rem; margin: 0; font-weight: 900; letter-spacing: 2px;">نظام حوكمة</h1>
            <p style="color:{S}; margin-top: 0.5rem; font-size: 0.95rem; letter-spacing: 2px; font-weight: 500; text-transform: uppercase;">
        ...
        <div style="text-align:center; margin-bottom: 2rem; margin-top: 0.5rem;">
            ...
            <h2 style="color:{T}; margin:0; font-weight:900; font-size:2rem; letter-spacing: 1px;">تأمين الهوية الرقمية</h2>
            <p style="color:{S}; margin-top:0.5rem; font-size:0.95rem; letter-spacing: 0.5px;">يرجى تحديث رمز الحماية الخاص بك للمتابعة</p>
        </div>
        """, unsafe_allow_html=True)
...
def login_form():
    ...
    # استخدام الأعمدة للتحكم في عرض الواجهة (تتجاوب تلقائياً مع الجوال)
    spacer_left, main_col, spacer_right = st.columns([1, 2.5, 1])
    ...
    <div style="text-align:center; margin-top: 4rem; margin-bottom: 1rem;">
        <p style="color: rgba(255,255,255,0.15); font-size: 0.75rem; letter-spacing: 2px; font-weight: 400;">
